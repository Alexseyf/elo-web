...
function ProfessorAtividadesContent() {
  ...
  const loadAtividades = async (professorId: number) => {
    setLoading(true);
    setError(null);

    try {
      const result = await fetchProfessorTurmaAtividades(professorId);
  ...
  const getAtividadesFiltradas = () => {
    return atividades.filter((atividade) => {
      const turmaMatch = !filtroTurma || String(atividade.turma.id) === filtroTurma;
    ...
    <div className="flex min-h-screen bg-gray-50 relative">
      <Sidebar
        ...
        setActiveSection={(section) => {
          router.push(`/professor/dashboard?section=${section}`);
        }}
        ...
        <main className="p-4 md:pt-6 lg:p-8">
          <div className="mb-6">
            <h2 className="text-2xl md:text-3xl font-semibold text-gray-900">
              Minhas Atividades Pedagógicas
            </h2>
            <p className="text-sm md:text-base text-gray-600 mt-1">
              Visualize suas atividades cadastradas
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded mb-4">
              ...
              <button
                onClick={() => userData?.id && loadAtividades(userData.id)}
                    ...
                    <tr key={atividade.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 md:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button
                          ...
                          onClick={() =>
                            router.push(`/professor/atividades/${atividade.id}`)
                          }
                        >
                          Detalhar
                        </button>
